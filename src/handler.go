...
func getVolumeInfo() map[string]int64 {
	...
	APP := tags["APP"]
	STAGE := tags["Stage"]
	parameter := fmt.Sprintf("/%s/%s/driveinfo", STAGE, APP)
	driveMap := make(map[string]int64)
...
func createFS(mPoint string, filesystem string) {
	fullPoint := fmt.Sprintf("/dev/group1/%s", mPoint)
	fileSystemCommand := fmt.Sprintf("mkfs.%s", filesystem)
	_, err := exec.Command(fileSystemCommand, fullPoint).Output()
	if err != nil {
		log.Fatal(err)
	}
}
